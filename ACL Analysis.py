...
def analyze_acl_configuration(acl_data):
    acl_results = []

    for config_file in acl_data:
    ...
    excel_file = 'acl_analysis.xlsx'
    df.to_excel(excel_file, index=False)

    print(f"ACL analysis results exported to {excel_file}")

def analyze_acls(acls, acl_results):
    for acl in acls:
...
# Usage: Provide the path to the exported configuration files
acl_data = ['describe_network_acls.json']
analyze_acl_configuration(acl_data)
